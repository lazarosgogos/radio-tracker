...
import {
  buildStationDateOptions,
  hasStationArchiveFilters,
  LAST_MINUTE_OF_DAY,
  type StationArchiveFilters
...
export function LiveStationArchive({ station: initialStation, plays: initialPlays, error, filters }: LiveStationArchiveProps) {
  ...
  const dayStartRef = useRef<HTMLInputElement>(null);
  const dayEndRef = useRef<HTMLInputElement>(null);
  const dayLabelRef = useRef<HTMLElement>(null);
  const timeStartRef = useRef<HTMLInputElement>(null);
  const timeEndRef = useRef<HTMLInputElement>(null);
  const timeLabelRef = useRef<HTMLElement>(null);
  ...
  const dayRangeLabel = `${formatDayLabel(dayOptions[dayRange[0]])} to ${formatDayLabel(dayOptions[dayRange[1]])}`;

  function setDayStart(value: number) {
    setDayRange((current) => [Math.min(value, current[1]), current[1]]);
  }

  function setDayEnd(value: number) {
    setDayRange((current) => [current[0], Math.max(value, current[0])]);
  }

  function setTimeStart(value: number) {
    setTimeRange((current) => [value, current[1]]);
  }

  function setTimeEnd(value: number) {
    setTimeRange((current) => [current[0], value]);
  }
  ...
  useEffect(() => {
    ...
    const dayEnd = dayEndRef.current;
    const timeStart = timeStartRef.current;
    const timeEnd = timeEndRef.current;

    function updateDays() {
      if (!dayStart || !dayEnd || !dayLabelRef.current) return;

      const start = Math.min(readRangeValue(dayStart, 0), readRangeValue(dayEnd, dayOptions.length - 1));
      const end = Math.max(readRangeValue(dayStart, 0), readRangeValue(dayEnd, dayOptions.length - 1));
      dayLabelRef.current.textContent = `${formatDayLabel(dayOptions[start])} to ${formatDayLabel(dayOptions[end])}`;
      setDayRange([start, end]);
    }

    function updateTime() {
      if (!timeStart || !timeEnd || !timeLabelRef.current) return;

      const start = readRangeValue(timeStart, 0);
      const end = readRangeValue(timeEnd, LAST_MINUTE_OF_DAY);
      const label = `${formatMinuteOfDay(start)} to ${formatMinuteOfDay(end)}`;
      timeLabelRef.current.textContent = start > end ? `${label} overnight` : label;
      setTimeRange([start, end]);
    }

    const dayInputs = [dayStart, dayEnd].filter((input): input is HTMLInputElement => Boolean(input));
    const timeInputs = [timeStart, timeEnd].filter((input): input is HTMLInputElement => Boolean(input));
    const dayEvents = ["input", "change", "pointermove", "pointerup", "touchmove", "touchend"];

    for (const input of dayInputs) {
      for (const eventName of dayEvents) {
        input.addEventListener(eventName, updateDays, { passive: true });
      }
    }

    for (const input of timeInputs) {
      for (const eventName of dayEvents) {
        input.addEventListener(eventName, updateTime, { passive: true });
      }
    }
    ...
    return () => {
      for (const input of dayInputs) {
        for (const eventName of dayEvents) {
          input.removeEventListener(eventName, updateDays);
        }
      }

      for (const input of timeInputs) {
        for (const eventName of dayEvents) {
          input.removeEventListener(eventName, updateTime);
        }
      }
    };
  }, [dayOptions]);
      ...
      <form className="archive-filters" action={`/stations/${station.slug}`}>
        ...
        <div className="range-panel">
          <div className="range-heading">
            <span>Days</span>
            <strong ref={dayLabelRef}>{dayRangeLabel}</strong>
          </div>
          <label className="range-control">
            ...
            <input
              ...
              step="1"
              value={dayRange[0]}
              aria-valuetext={formatDayLabel(dayOptions[dayRange[0]])}
              onInput={(event) => setDayStart(Number(event.currentTarget.value))}
              onChange={(event) => setDayStart(Number(event.currentTarget.value))}
            />
          </label>
          <label className="range-control">
            <span>End day</span>
            <input
              ...
              step="1"
              value={dayRange[1]}
              aria-valuetext={formatDayLabel(dayOptions[dayRange[1]])}
              onInput={(event) => setDayEnd(Number(event.currentTarget.value))}
        ...
        <div className="range-panel">
          <div className="range-heading">
            <span>Time</span>
            <strong ref={timeLabelRef}>{timeRangeLabel}</strong>
          </div>
          <label className="range-control">
            ...
            <input
              ...
              step="5"
              value={timeRange[0]}
              aria-valuetext={todFrom}
              onInput={(event) => setTimeStart(Number(event.currentTarget.value))}
              onChange={(event) => setTimeStart(Number(event.currentTarget.value))}
            />
          </label>
          <label className="range-control">
            <span>End time</span>
            <input
              ...
              step="5"
              value={timeRange[1]}
              aria-valuetext={todTo}
              onInput={(event) => setTimeEnd(Number(event.currentTarget.value))}
              onChange={(event) => setTimeEnd(Number(event.currentTarget.value))}
            />
          </label>
        </div>
...
function readRangeValue(input: HTMLInputElement, fallback: number): number {
