...
export async function listStationPlays(slug: string, search?: string): Promise<PlayRow[]> {
  const params: unknown[] = [slug];
  const searchClause = search?.trim()
    ? "AND (coalesce(t.artist, '') || ' ' || coalesce(t.title, '') || ' ' || t.raw_text) % $2"
    : "";
  ...
  const result = await query<PlayRow>(
    `
      ...
      WHERE p.played_at >= now() - interval '30 days'
        AND (
          (coalesce(t.artist, '') || ' ' || coalesce(t.title, '') || ' ' || t.raw_text || ' ' || s.name) % $1
          OR s.name ILIKE '%' || $1 || '%'
        )
      ORDER BY
